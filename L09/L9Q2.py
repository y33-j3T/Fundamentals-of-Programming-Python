...
def encode(code, msg):
    ''' Given parameters code and msg, take the arguments:
        code: this is a dictionary describing a code
        msg: this is a string
        
        Apply the mapping to each character of the string as 
        described above and return the encoded output string '''
        
    encoded_msg = '' 
    for i in range(len(msg)):
        if(msg[i] not in code.keys()):
            encoded_msg += msg[i]
        else:
...
print(encode(code0(), some_string))
print(encode(code1(), some_string))
print(encode(code2(), some_string))
print(encode(code3(), some_string))
